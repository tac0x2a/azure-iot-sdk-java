...
public class MqttDeviceTwinTest
{
    ...
    **Tests_SRS_MqttDeviceTwin_25_031: [**send method shall publish a message to the IOT Hub on the respective publish topic by calling method publish().**]**
     */
    @Test
    public void sendPublishesMessageForSubscribeToDesiredPropertiesOnCorrectTopic(@Mocked final Mqtt mockMqtt, @Mocked final DeviceTwinMessage mockMessage) throws IOException
    {
        //arrange
        final byte[] actualPayload = {0x61, 0x62, 0x63};
    ...
    **Tests_SRS_MqttDeviceTwin_25_041: [**If the topic is of type response topic then this method shall parse further to look for version which if found is set by calling setVersion**]**
    ...
    {
        final byte[] actualPayload = "GetTwinResponseDataContainingDesiredAndReportedPropertiesDocument".getBytes();
        final String expectedTopic = "$iothub/twin/res/" + "201" + "/?$version=" + mockVersion;
        DeviceTwinMessage receivedMessage = null;
        try
        {
            //arrange
    ...
    **Tests_SRS_MqttDeviceTwin_25_041: [**If the topic is of type response topic then this method shall parse further to look for version which if found is set by calling setVersion**]**
    ...
    {
        final byte[] actualPayload = "GetTwinResponseDataContainingDesiredAndReportedPropertiesDocument".getBytes();
        final String expectedTopic = "$iothub/twin/res/" + "201";
        DeviceTwinMessage receivedMessage = null;
        try
        {
            //arrange
    ...
    **Tests_SRS_MqttDeviceTwin_25_034: [**If the call parseTopic returns null or empty string then this method shall do nothing and return null**]**
    ...
    /*
    **Tests_SRS_MqttDeviceTwin_25_015: [**This onReconnect method shall put the entire operation of the MqttDeviceTwin class on hold by acquiring a binary semaphore.**]**
    */
    @Test
    public void OnReconnectStopsAllOperation(@Mocked final Mqtt mockMqtt) throws IOException
    {
        MqttDeviceTwin testTwin = new MqttDeviceTwin();
        testTwin.onReconnect();
        Semaphore actualSemaphore = Deencapsulation.getField(testTwin, "DEVICE_TWIN_MESSAGING_SEMAPHORE");
        assertTrue(actualSemaphore.availablePermits() == 0);
    }
    /*
    **Tests_SRS_MqttDeviceTwin_25_017: [**If the status is true, onReconnectComplete method shall release all the operation of the MqttDeviceTwin class put on hold by releasing the semaphore.**]**
     */
    @Test
    public void OnReconnectCompleteResumesAllOperation(@Mocked final Mqtt mockMqtt) throws IOException
    {
        MqttDeviceTwin testTwin = new MqttDeviceTwin();
        testTwin.onReconnect();
        testTwin.onReconnectComplete(true);
        Semaphore actualSemaphore = Deencapsulation.getField(testTwin, "DEVICE_TWIN_MESSAGING_SEMAPHORE");
